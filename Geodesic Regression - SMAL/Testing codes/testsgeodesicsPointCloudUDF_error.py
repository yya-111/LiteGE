...
gtdata = np.load("distancestestremesh.npz")['arr_0'] #decompress_lzma_to_numpy("distancestestremesh.npy.lzma")
#if gtdata:
#     # Assuming original dtype was float16 and shape (101, 2000, 6890)
#    original_dtype_array1 = np.float16
#    original_shape_array1 = (100,2700,5356)
#    gtdata = 1.42*np.frombuffer(gtdata, #dtype=original_dtype_array1).reshape(original_shape_array1)
...
#meshpca = (np.load(testdata)['testpca'] )*stdpca + meanpca
#meshpca = meshpca[:,:320]


mean_coord = np.array([ 8.4521987e-02, -8.0156110e-02 ,-3.9249542e-05] )[None,None,:]
std_coord = np.array([0.31262797, 0.18953875, 0.07636771])[None,None,:]
...
#meshverticesori = np.load("MeshVerticesSMALProcessed.npz")['arr_0'][0:1000]
...
#faces = trimesh.load("MeshTestSample" + os.sep + "meshtest_4_ori.off", process=False).faces
...
for i in range(meshverticesori.shape[0]):
    # Suppose meshverticesori[i] is your (N, 3) point cloud
    points = meshverticesori[i]  # shape (N, 3)
    ...
    meshverticesori[i] = rotated_points
    
    vertices = torch.from_numpy(meshverticesori[i]).cuda()
    #noise_std = 0.01  # Adjust as needed
    #vertices = vertices + torch.randn_like(vertices) * noise_std
    
    sampled_pts = sample_mesh(vertices, faces, sample_count = 2000)
    with torch.no_grad():
    ...
    meshverticesori[i] = vertices.detach().cpu().numpy()
    gtdata[i] *= scale
...
#
#print(pointclouds_testori.shape)
important_points_batch =  np.load("VoxelsCenterNearSurfaceHighstd)PointCloudUDF_0_16fil.npy")
...
meshvertices = (meshverticesori- mean_coord)/std_coord
...
meshpcaactual = np.load("PCA_PointCloudUDF_dense_rot_center_cleannoise_all_shapematch_0_16fil.npy")
testidx = np.load(testidxname)
meshpcaactual = meshpcaactual[testidx][:, :240]
print("MeshPCA shape:",meshpcaactual.shape, meshpca.shape)
x_norm = F.normalize(torch.from_numpy(meshpcaactual), dim=1)
y_norm = F.normalize(torch.from_numpy(meshpca), dim=1)
cosine_sim = (x_norm * y_norm).sum(dim=1)
print("Cosie similarity:",cosine_sim)
...
for b in range(NTEST[0]):
    m1 = np.full((1, K), b, dtype=np.int32)
    v1 = rng.integers(0, sources.shape[0], size=(1,K))
    v2 = rng.integers(0, meshvertices[0].shape[0], size=(1,K))
...
error = np.mean(np.abs(resultpred - resulttarget))
print("Error:",error)
