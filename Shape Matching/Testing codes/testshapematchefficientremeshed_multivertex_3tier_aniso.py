...
for i in range(meshvertices.shape[0]):
    ...
    verts_aniso = torch.tensor(mesh_i_aniso.vertices).float().cuda()
    faces_aniso = torch.tensor(mesh_i_aniso.faces).long().cuda()
    ...
    vertices, scale_factor, pc_standard = utils.processmesh_torch(verts_aniso, faces_aniso)
    ...
    meshvertices[i] = vertices.cpu().numpy()[:meshvertices[i].shape[0]]
...
with torch.no_grad():
    for i in tqdm(range(NTEST), mininterval=30):
        ...
        # recover which mesh1, mesh2, vertex1 this batch corresponded to:
        # (we stored them in the dataset’s index arrays)
        # i = the global sample index of the first element in this batch
        # but DataLoader with batch_size=B and shuffle=False will give batches
        # in order: batch 0 covers idx 0…B-1, batch 1 covers B…2B-1, etc.
