...
def train(args):
    ...
    huberloss = torch.nn.HuberLoss(reduction='mean', delta=0.05)
    ...
    train_data = load_data(meshvertices, npzdatatrain, pca_path, num_workers=8, batch_size = args.batch_size)
    print("Data loaded")
    global_step = 0
    global_step_val = 0
    ...
    for epoch in range(startepoch, args.num_epoch):
        ...
        for mesh1, source, dest, dist1 in tqdm(train_data, miniters=2000, mininterval=35):
            ...
            #threshold = torch.quantile(loss_val.flatten(), 0.9)
            #mask = loss_val <= threshold
            #filtered_loss_val = loss_val[mask]
            #print(mask.shape)
        ...
        if(epoch % val_epoch == 0):
            ...
            with torch.no_grad():
                ...
                # Mean
                overall_mean_per_item_loss = sum_losses_scalar / total_items
                ...
                overall_std_per_item_loss = np.sqrt(variance)
                ...
                # Check improvements
                if (overall_mean_per_item_loss < best_val_loss or overall_std_per_item_loss < best_std or median_loss < best_med_loss):
                    print("Best Val Loss or Val Std improved")
                    best_med_loss = median_loss
                    best_val_loss = overall_mean_per_item_loss
                    best_std = overall_std_per_item_loss

                    save_model(model, optimizer, scheduler, epoch,
                               f"pcamodel_ep_{epoch}_l_{overall_mean_per_item_loss}_std_{overall_std_per_item_loss}_med_{median_loss}")
