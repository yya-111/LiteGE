...
class MCAGeodesicData(Dataset):
    ...
    def __init__(self, npz_meshvertices, npz_data_path, pca_path, scalefactor_path):
        ...
        self.vertices = np.load(npz_meshvertices)['arr_0'].astype(np.float32)
        ...
        mesh_b_sources = self.vertices[self.mesh_b, self.sources]
        mesh_b_dests = self.vertices[self.mesh_b, self.dests]
        mesh_b_euclid = 1.42 *self._compute_euclidean( mesh_b_sources, mesh_b_dests)
        print(mesh_b_euclid.shape)
        del mesh_b_sources , mesh_b_dests
        self.dist_on_a -= mesh_a_euclid
        self.dist_on_b -= mesh_b_euclid
        del mesh_a_euclid, mesh_b_euclid
