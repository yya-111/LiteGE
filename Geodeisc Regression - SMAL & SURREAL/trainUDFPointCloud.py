...
class WeightedL1_piecewise(nn.Module):
    def __init__(self, y0_low=0.08, w_low=40, y0_high=0.17, w_mid=25, w_high=1.0, reduction='mean'):
        super().__init__()
        self.y0_low, self.w_low = y0_low, w_low
        self.y0_high, self.w_mid = y0_high, w_mid
        self.w_high = w_high
...
def train(args):
    ...
    model = CoordMLP(neurons=args.no_neurons, embedding_size=240).to(device)
    ...
    pca_path = "PCA_SMALSurrealPointCloudUDF_dense_rot_center_cleannoise_all_0_12fil.npy"#"PCA_SMALSurrealPointCloudUDF_dense_rot_center_cleannoise_all_0_025fil.npy"
    scalefactor_path = ["ScaleSurrealFromUnitAreaMesh_UDFPointCloud.npz.npy","/notebooks/ScaleFromRandomMesh_UDFPointCloud.npz.npy"] 
    
    meshvertices = ["SurrealVertices_UDFFromPointCloud.npz","/notebooks/SMALVertices_UDFFromPointCloud.npz"]
    
    npzdatatrain = ["geodesic_data_human8K_0.npz", "geodesic_smal_8k_train.npz"]
    npzdataval = ["geodesic_data_human8K_val.npz","geodesic_smal_8k_val.npz"]
    ...
    for epoch in range(startepoch, args.num_epoch):
        ...
        #if(epoch >= 1):
        #    break
        for mesh1, source, dest, dist1 in tqdm(train_data, miniters=2000, mininterval=35):
            
            nobatch+=1
            mesh1, source, dest, dist1 = mesh1.to(device, non_blocking=True), source.to(device, non_blocking=True), \
                  dest.to(device, non_blocking=True), dist1.to(device, non_blocking=True)

            pred = model(source,dest, mesh1, mesh1)
            #gt = torch.cat((dist1,dist2),dim=-1)
            loss_val = loss(pred,dist1)#(huberloss(pred, (dist1 +dist2)/2.0))
        ...
        if(epoch%val_epoch==0):
            ...
            with torch.no_grad():
                for mesh1, source, dest, dist1 in tqdm(valid_data):
                    ...
                    nobatch+=1
                    mesh1, source, dest, dist1 = mesh1.to(device, non_blocking=True),source.to(device, non_blocking=True), dest.to(device, non_blocking=True), dist1.to(device, non_blocking=True)

                    pred = model(source,dest, mesh1, mesh1)
                    
                    target = (dist1 )# Your target calculation
                    #gt = torch.cat((dist1,dist2),dim=-1)
                    

            
                    per_item_losses = lossval(pred,target)#lossval(pred, target) ### Assumes loss(reduction='none')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-neu', '--no_neurons', type=int,default=200)
